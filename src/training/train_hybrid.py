...
from src.models.resnet_hybrid import ResNetHybrid
...
def train(
    ...
    model = ResNetHybrid(num_classes=num_classes, in_channels=in_channels).to(device)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train ResNet-Hybrid (ResNet18 + Transformer head) on FER data.")
